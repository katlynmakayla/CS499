...
router
  ...
  .get(authenticateJWT, (req, res, next) => {
    ...
    next();
  }, recommendationsController.getRecommendations); // GET Method routes Trip Recommendations

// Booking routes
router
  .route("/bookings")
  .post(authenticateJWT, bookingsController.bookingsCreate); // Create booking (any authenticated user)
router
  .route("/bookings/mine")
  .get(authenticateJWT, bookingsController.bookingsMine); // Get my own bookings
router
  .route("/bookings/all")
  .get(authenticateJWT, requireAdmin, bookingsController.bookingsAll); // Get all bookings (admin only)
router
  .route("/bookings/:id/cancel")
  .patch(authenticateJWT, bookingsController.bookingsCancel); // Cancel a booking (owner or admin)

// User profile routes
router
    .route('/user')
    .get(authenticateJWT, usersController.getProfile); // Get own profile data (including preferences)
router
  .route("/user/profile")
  .put(authenticateJWT, usersController.updateProfile); // Update own profile preferences
